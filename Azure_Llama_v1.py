"""
description: A pipeline for generating text using the Llama model via Azure AI Inference API, with fixed stream handling.
...
class Pipeline:
    ...
    class Valves(BaseModel):
        ...
        MODEL_ID: str = "Llama-30b-chat"

    def __init__(self):
        self.type = "manifold"
        self.id = "Llama-azure"
        self.name = "Llama-azure/"
        ...
        self.valves = self.Valves(
            **{
                ...
                "MODEL_ID":
                os.getenv("MODEL_ID", "Llama-30b-chat"),
            })
        self.update_client()
    ...
    def pipe(self, user_message: str, model_id: str, messages: List[dict],
        ...
        try:
            ...
            logger.debug(f"Prepared Llama messages: {Llama_messages}")
            logger.debug(f"Filtered body: {filtered_body}")

            is_stream = body.get("stream", False)
            if is_stream:
                return self.stream_response(Llama_messages, filtered_body)
            else:
                return self.get_completion(Llama_messages, filtered_body)
        except Exception as e:
            logger.error(f"Error in pipe: {str(e)}", exc_info=True)
            return json.dumps({"error": str(e)})

    def stream_response(self, Llama_messages: List[Union[SystemMessage, UserMessage, AssistantMessage]], params: dict) -> str:
        try:
            complete_response = ""
            response = self.client.complete(messages=Llama_messages,
    ...
    def get_completion(self, Llama_messages: List[Union[SystemMessage, UserMessage, AssistantMessage]], params: dict) -> str:
        try:
            response = self.client.complete(messages=Llama_messages,
                                            model=self.valves.MODEL_ID,
                                            **params)
